...
fn main() {
    let fsdiag_app = clap::App::new("File system diag")
                            ...
                            .arg(clap::Arg::with_name("compare")
                                    ...
                                    .value_name("FILE")
                                    .conflicts_with_all(&["new","output","extension","symlink"])
                                )
...
fn output_write(files: &mut Vec<std::path::PathBuf>,hashes: &mut Vec<String>,fname: Option<&str>){
    if files.is_empty() { println!("{}[-]{} No files found matching specified criteria.",RED,DEFAULT); return; }
...
fn compare(f: &str,p: &std::path::Path) {
    ...
    let cf = match fs::File::open(f){
        Ok(_f) => _f,
        Err(_f) => {println!("{}[-]{} Failed to open - {}",RED,DEFAULT,f);return;}
    };
    ...
    if files.is_empty() { println!("{}[-]{} No files found with the specified path.",RED,DEFAULT );return; }
    for f in files.iter_mut() {
        ...
        match fs::File::open(nf){
            ...
            Err(_) => {
                if f[1] == "None".to_owned() { println!("{}[-]{} Failed to hash - {}.",RED,DEFAULT,f[0] );}
                else {println!("{}[-]{} File {} - removed or renamed.",RED,DEFAULT,f[0]);} f_changed += 1;  },
        }
    }
    if f_changed == 0 { println!("{}[+]{} No files were modified.",GREEN, DEFAULT);}
    else if files.len() != 1 { println!("{}[+]{} All other files were not changed.",GREEN, DEFAULT); }
}
